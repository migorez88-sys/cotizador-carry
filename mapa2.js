...
const ModuloMapa = {
    ...
    crearIconoColor: function(colorClase) {
        return L.divIcon({
            className: `custom-pin-icon ${colorClase}`,
            ...
            html: `<svg xmlns="http://w3.org" viewBox="0 0 24 24" class="svg-marker-pin">
                    <!-- Sombra sutil del pin en el mapa -->
                    <ellipse cx="12" cy="22" rx="5" ry="2" fill="rgba(0,0,0,0.25)" />
                    <!-- Cuerpo del marcador tradicional de mapas -->
    ...
    init: function () {
        ...
        window.addEventListener('message', (evento) => {
            ...
            // 🚀 NUEVA CONDICIÓN: Si el usuario eligió de favoritos, pintamos el pin de golpe
            if (evento.data.tipo === 'PIN_DESDE_DATALIST') {
                ...
                // Creamos una lista con las coordenadas de todos los pines que YA están dibujados
                const coordenadasActivas = [];
                for (const clave in this.marcadores) {
                    if (this.marcadores[clave] !== null) {
                        // Extraemos la latitud y longitud actual de cada marcador guardado
                        const posicionPin = this.marcadores[clave].getLatLng();
                        coordenadasActivas.push(posicionPin);
                    }
                }
                ...
                if (coordenadasActivas.length > 1) {
                    ...
                    this.instanciaMapa.fitBounds(limites, {
                        padding: [50, 50] ,  // Margen de seguridad en píxeles para que los pines no queden pegados a los bordes
                        maxZoom: 15    // Evita que si los puntos están exageradamente cerca haga un zoom ultra-profundo
                    });
                } else {
                    // Si es el primer y único pin en el mapa, hacemos un zoom estándar y suave
                    this.instanciaMapa.setView([latitud, longitud], 13);
                }
            }
        });
    },
